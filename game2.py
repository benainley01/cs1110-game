...
statue = gamebox.from_image(400,-1200,'homerstatue.png')
statue.scale_by(.5)
    ...
    if game_on2:
        ...
        for each in player:
            for wall in invis_wall:
                each.move_to_stop_overlapping(wall)
        for each in player:
            for roof in roofs:
                each.move_to_stop_overlapping(roof)
        for each in player:
